...
1. **パスベースのフィルタリング:**
  - 設定ファイル (`.coderabbitai.yaml` 相当) の `file_filter.exclude` に基づいて除外。(以前の `path_filters` は非推奨)
...
**Code Hedgehog との比較:** Code Hedgehog の `FileManager` も `file_filter.exclude` による除外を行いますが、現時点ではサイズや複雑さに基づく動的なフィルタリングや `skip_simple_changes` の実装は `Action Runner` レベルでは行われていません (プロセッサ側で考慮される可能性はあります)。
...
**Code Hedgehog との比較:** Code Hedgehog では、現時点ではこのような初期分析や選択的レビューの仕組みは `Action Runner` レベルでは実装されていません。`FileManager` が収集した全ファイル (`file_filter.exclude` で除外後) がプロセッサに渡され、プロセッサがレビューを実行します。サマリー生成 (`ProcessOutput.summary`) は、詳細レビューの結果としてプロセッサが行います。
...
1. **レビュールールの管理:**
   - `file_filter.exclude`, `file_path_instructions` などによるレビュー対象や指示のカスタマイズ。(以前の `path_filters` は非推奨)
