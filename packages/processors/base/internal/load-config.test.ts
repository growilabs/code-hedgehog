...
import { test, describe, afterEach } from '@std/testing/bdd'; // Removed beforeEach as stubs are created per test
...
import { DEFAULT_CONFIG, type ReviewConfig } from '../deps.ts';
...
describe('loadConfig', () => {
  ...
  test('should return default config and log error for invalid YAML format (parse error)', async () => {
    ...
    // Check for YAMLParseError instead of YAMLException
    expect(errorStub.calls[0].args[1].name).toBe('YAMLParseError');
